...
def AddNoise(snr, spectra):
    # compute the signals average power and use that to find the noise average power and SD
    P_signal = np.mean(spectra**2)
    #print("signal power: ", P_signal)
    P_noise = max(P_signal / snr, 0.001) # We still want noise if the signal is very low / 0
    #print("noise power: ", P_noise)
    sigma = np.sqrt(P_noise)

    #generate radnom noise according to above parameters
    noise = np.random.normal(0, sigma, size=spectra.shape)
    print("noise: ", noise)
    noisy = spectra + noise

    # ensure no values are negative
